...
figure()
plot3(shoulder_mat(ap,time_vec),shoulder_mat(ml,time_vec),shoulder_mat(si,time_vec),'o')
hold on 
plot3(elbow_mat(ap,time_vec),elbow_mat(ml,time_vec),elbow_mat(si,time_vec),'o')
plot3(wrist_mat(ap,time_vec),wrist_mat(ml,time_vec),wrist_mat(si,time_vec),'o')
plot3(hand_mat(ap,time_vec),hand_mat(ml,time_vec),hand_mat(si,time_vec),'o')
plot3(joystick_mat(ap,time_vec),joystick_mat(ml,time_vec),joystick_mat(si,time_vec),'o')
...
for i = 1:length(time_vec)
   plot3([shoulder_mat(ap,time_vec(i)) elbow_mat(ap,time_vec(i))],...
       [shoulder_mat(ml,time_vec(i)) elbow_mat(ml,time_vec(i))],...
       [shoulder_mat(si,time_vec(i)) elbow_mat(si,time_vec(i))],'color','k','LineWidth',1)
   plot3([elbow_mat(ap,time_vec(i)) wrist_mat(ap,time_vec(i))],...
       [elbow_mat(ml,time_vec(i)) wrist_mat(ml,time_vec(i))],...
       [elbow_mat(si,time_vec(i)) wrist_mat(si,time_vec(i))],'color','k','LineWidth',1)
