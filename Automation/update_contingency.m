...
date_data = datenum(dir_elements{end},'mmddyy');
...
if def_dir == 2
    ...
    % if (angle_l(1)<60)&&(angle_l(1)>0)&&(angle_l(2)>270)&&(angle_l(1)<360)
    %    angle_l(1) = angle_l(1) + 360;
    % end
    
    %if (angle_nl(1)<60)&&(angle_nl(1)>0)&&(angle_nl(2)>270)&&(angle_nl(1)<360)
    %    angle_nl(1) = angle_nl(1) + 360;
    %end
...
%     if ~no_prev_cont
%         for cont_ind = [1 4]
%             if (prev_cont(cont_ind)<60)&&(prev_cont(cont_ind)>0)&&(prev_cont(cont_ind+1)>270)&&(prev_cont(cont_ind+1)<360)
%                 prev_cont(cont_ind) = prev_cont(cont_ind) + 360;
%             end
%         end
%     end
    ...
    if split == 0
        ...
        if ~no_prev_cont && ~isequal(prev_cont(1:6),[0 360 1 0 360 1])
            angnl_temp = sort(angle_nl(1:2),1);
            pc_temp = sort(prev_cont(1:2));            
            %temp1 = sortrows([angle_nl(1:3);prev_cont(1:3)],-1);
            if def_dir == 1 %Go CCW
                if(pc_temp(1)-angnl_temp(1))<0||(pc_temp(1)-angnl_temp(1))>180
                    angle_nl(1:3) = angle_nl(1:3);
                else
                    angle_nl(1:3) = prev_cont(1:3);
                end
            elseif def_dir == 0                
                if(pc_temp(1)-angnl_temp(1))>0||(pc_temp(1)-angnl_temp(1))<180
                    angle_nl(1:3) = angle_nl(1:3);
