function [console_output] = multi_traj_maxvel(dirlist,varargin)
...
for i= 1:length(statslist)
    ...
    [~,maxvel{i}] = traj_maxvel(stats,0,rw_only,interv,ax,plotflag,colors(i));    
    console_output{i+1} = sprintf(strcat(dates{i},' Med: %d'),median([maxvel{i}]));
end

console_output{1} = 'Max Vel';
if (lasercompareflag-1)
    [h,p] = kstest2([maxvel{1}],[maxvel{2}]);
