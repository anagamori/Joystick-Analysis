...
for j = 1 %:length(index_reward) %1:50 %3:32
    ...
    for k = 94 %1:length(js_reward)
        ...
        if offset_js(k) == offset_trial
            ...
            plot(time(offset_js(k)-onset_js(k)),0.7,'.k', 'MarkerSize',10)
            xlim([time(1)-0.1,time(end)])
            legend('Reward','Trial Onset','Trial Offset','Joystick Offest')
            set(gca,'TickDir','out');
            set(gca,'box','off')
        elseif offset_np == offset_trial
            figure(k)
            subplot(4,1,1)
            plot(time,traj_x(start_time:end_time),'LineWidth',1)
            xlim([time(1),time(end)])
            ...
            plot(0,0.3,'.b', 'MarkerSize',10)
            plot(time(offset_trial-onset_js(k)),0.5,'.m', 'MarkerSize',10)
            plot(time(offset_np-onset_js(k)),0.7,'.k', 'MarkerSize',10)
            xlim([time(1)-0.1,time(end)])
            legend('Reward','Trial Onset','Trial Offset','Nose Poke Offest')
            set(gca,'TickDir','out');
            set(gca,'box','off')
        elseif offset_js_l == offset_trial
            figure(k)
            subplot(4,1,1)
            plot(time,traj_x(start_time:end_time),'LineWidth',1)
            xlim([time(1),time(end)])
            ...
            xlim([time(1)-0.1,time(end)])
            legend('Reward','Trial Onset','Trial Offset','Touch 2 Offest')
            set(gca,'TickDir','out');
            set(gca,'box','off')
        end
%   
