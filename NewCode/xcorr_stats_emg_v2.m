...
for i = 1:nIteration
    ...
    EMG_triceps = EMG_biceps_ds_all(idx_sample,:);
    EMG_a_delt = EMG_a_delt_ds_all(idx_sample,:);
    EMG_p_delt = EMG_p_delt_ds_all(idx_sample,:);
    
    for j = 1:5
        [r_biceps_triceps,lags] = xcorr(EMG_biceps(j,:)-mean(EMG_biceps(j,:)),EMG_triceps(j,:)-mean(EMG_triceps(j,:)),window_size,'coeff');
        r_biceps_triceps_all_temp = [r_biceps_triceps_all_temp;r_biceps_triceps];
        [r_biceps_a_delt,~] = xcorr(EMG_biceps(j,:)-mean(EMG_biceps(j,:)),EMG_a_delt(j,:)-mean(EMG_a_delt(j,:)),window_size,'coeff');
        r_biceps_a_delt_all_temp = [r_biceps_a_delt_all_temp;r_biceps_a_delt];
        [r_biceps_p_delt,~] = xcorr(EMG_biceps(j,:)-mean(EMG_biceps(j,:)),EMG_p_delt(j,:)-mean(EMG_p_delt(j,:)),window_size,'coeff');
        r_biceps_p_delt_all_temp = [r_biceps_p_delt_all_temp;r_biceps_p_delt];
        
        [r_triceps_a_delt,~] = xcorr(EMG_triceps(j,:)-mean(EMG_triceps(j,:)),EMG_a_delt(j,:)-mean(EMG_a_delt(j,:)),window_size,'coeff');
        r_triceps_a_delt_all_temp = [r_triceps_a_delt_all_temp;r_triceps_a_delt];   
        [r_triceps_p_delt,~] = xcorr(EMG_triceps(j,:)-mean(EMG_triceps(j,:)),EMG_p_delt(j,:)-mean(EMG_p_delt(j,:)),window_size,'coeff');
        r_triceps_p_delt_all_temp = [r_triceps_p_delt_all_temp;r_triceps_p_delt];
        
        [r_a_delt_p_delt,~] = xcorr(EMG_a_delt(j,:)-mean(EMG_a_delt(j,:)),EMG_p_delt(j,:)-mean(EMG_p_delt(j,:)),window_size,'coeff');
        r_a_delt_p_delt_all_temp = [r_a_delt_p_delt_all_temp;r_a_delt_p_delt];
        
    end
    r_biceps_triceps_all = [r_biceps_triceps_all;mean(r_biceps_triceps_all_temp)];
    r_biceps_a_delt_all = [r_biceps_a_delt_all;mean(r_biceps_a_delt_all_temp)];
    r_biceps_p_delt_all = [r_biceps_p_delt_all;mean(r_biceps_p_delt_all_temp)];
    r_triceps_a_delt_all = [r_triceps_a_delt_all;mean(r_triceps_a_delt_all_temp)];   
    r_triceps_p_delt_all = [r_triceps_p_delt_all;mean(r_triceps_p_delt_all_temp)];   
    r_a_delt_p_delt_all = [r_a_delt_p_delt_all;mean(r_a_delt_p_delt_all_temp)];
...
r_biceps_triceps_rand_all = [];
r_biceps_a_delt_rand_all = [];
r_biceps_p_delt_rand_all = [];
r_triceps_a_delt_rand_all = [];
r_triceps_p_delt_rand_all = [];
r_a_delt_p_delt_rand_all = [];

r_biceps_triceps_rand_all_temp = [];
r_biceps_a_delt_rand_all_temp = [];
r_biceps_p_delt_rand_all_temp = [];
r_triceps_a_delt_rand_all_temp = [];
r_triceps_p_delt_rand_all_temp = [];
r_a_delt_p_delt_rand_all_temp = [];

for i = 1:nIteration
    idx_sample = datasample(s,1:size(combinations,1),5,'Replace',true);

    EMG_biceps = EMG_biceps_ds_all(combinations(idx_sample,1),:);
    EMG_triceps = EMG_biceps_ds_all(combinations(idx_sample,2),:);
    EMG_a_delt = EMG_a_delt_ds_all(combinations(idx_sample,2),:);
    EMG_p_delt = EMG_p_delt_ds_all(combinations(idx_sample,2),:);
    
    for j = 1:5
        [r_biceps_triceps_rand,lags] = xcorr(EMG_biceps(j,:)-mean(EMG_biceps(j,:)),EMG_triceps(j,:)-mean(EMG_triceps(j,:)),window_size,'coeff');
        r_biceps_triceps_rand_all_temp = [r_biceps_triceps_rand_all_temp;r_biceps_triceps_rand];
        [r_biceps_a_delt_rand,~] = xcorr(EMG_biceps(j,:)-mean(EMG_biceps(j,:)),EMG_a_delt(j,:)-mean(EMG_a_delt(j,:)),window_size,'coeff');
        r_biceps_a_delt_rand_all_temp = [r_biceps_a_delt_rand_all_temp;r_biceps_a_delt_rand];
        [r_biceps_p_delt_rand,~] = xcorr(EMG_biceps(j,:)-mean(EMG_biceps(j,:)),EMG_p_delt(j,:)-mean(EMG_p_delt(j,:)),window_size,'coeff');
        r_biceps_p_delt_rand_all_temp = [r_biceps_p_delt_rand_all_temp;r_biceps_p_delt_rand];
    end
      
    EMG_triceps = EMG_biceps_ds_all(combinations(idx_sample,1),:);
    EMG_a_delt = EMG_a_delt_ds_all(combinations(idx_sample,2),:);
    EMG_p_delt = EMG_p_delt_ds_all(combinations(idx_sample,2),:);    
    for j = 1:5
        [r_triceps_a_delt_rand,~] = xcorr(EMG_triceps(j,:)-mean(EMG_triceps(j,:)),EMG_a_delt(j,:)-mean(EMG_a_delt(j,:)),window_size,'coeff');
        r_triceps_a_delt_rand_all_temp = [r_triceps_a_delt_rand_all_temp;r_triceps_a_delt_rand];   
        [r_triceps_p_delt_rand,~] = xcorr(EMG_triceps(j,:)-mean(EMG_triceps(j,:)),EMG_p_delt(j,:)-mean(EMG_p_delt(j,:)),window_size,'coeff');
        r_triceps_p_delt_rand_all_temp = [r_triceps_p_delt_rand_all_temp;r_triceps_p_delt_rand];     
    end
       
    EMG_a_delt = EMG_a_delt_ds_all(combinations(idx_sample,1),:);
    EMG_p_delt = EMG_p_delt_ds_all(combinations(idx_sample,2),:);    
    for j = 1:5
        [r_a_delt_p_delt_rand,~] = xcorr(EMG_a_delt(j,:)-mean(EMG_a_delt(j,:)),EMG_p_delt(j,:)-mean(EMG_p_delt(j,:)),window_size,'coeff');
        r_a_delt_p_delt_rand_all_temp = [r_a_delt_p_delt_rand_all_temp;r_a_delt_p_delt_rand];
    end
    
    r_biceps_triceps_rand_all = [r_biceps_triceps_rand_all;mean(r_biceps_triceps_rand_all_temp)];
    r_biceps_a_delt_rand_all = [r_biceps_a_delt_rand_all;mean(r_biceps_a_delt_rand_all_temp)];
    r_biceps_p_delt_rand_all = [r_biceps_p_delt_rand_all;mean(r_biceps_p_delt_rand_all_temp)];
    r_triceps_a_delt_rand_all = [r_triceps_a_delt_rand_all;mean(r_triceps_a_delt_rand_all_temp)]; 
    r_triceps_p_delt_rand_all = [r_triceps_p_delt_rand_all;mean(r_triceps_p_delt_rand_all_temp)]; 
    r_a_delt_p_delt_rand_all = [r_a_delt_p_delt_rand_all;mean(r_a_delt_p_delt_rand_all_temp)];
...
y_biceps_triceps_rand_1 = mean(r_biceps_triceps_rand_all);
se_biceps_triceps_rand_1 = std(r_biceps_triceps_rand_all,[],1)./(sqrt(size(r_biceps_triceps_rand_all,1)));
y_biceps_a_delt_rand_1 = mean(r_biceps_a_delt_rand_all);
se_biceps_a_delt_rand_1 = std(r_biceps_a_delt_rand_all,[],1)./(sqrt(size(r_biceps_a_delt_rand_all,1)));
y_biceps_p_delt_rand_1 = mean(r_biceps_p_delt_rand_all);
se_biceps_p_delt_rand_1 = std(r_biceps_p_delt_rand_all,[],1)./(sqrt(size(r_biceps_p_delt_rand_all,1)));
...
y_triceps_a_delt_rand_1 = mean(r_triceps_a_delt_rand_all);
se_triceps_a_delt_rand_1 = std(r_triceps_a_delt_rand_all,[],1)./(sqrt(size(r_triceps_a_delt_rand_all,1)));
...
y_triceps_p_delt_rand_1 = mean(r_triceps_p_delt_rand_all);
se_triceps_p_delt_rand_1 = std(r_triceps_p_delt_rand_all,[],1)./(sqrt(size(r_triceps_p_delt_rand_all,1)));

y_a_delt_p_delt_1 = mean(r_a_delt_p_delt_all);
se_a_delt_p_delt_1 = std(r_a_delt_p_delt_all,[],1)./(sqrt(size(r_a_delt_p_delt_all,1)));
y_a_delt_p_delt_rand_1 = mean(r_a_delt_p_delt_rand_all);
se_a_delt_p_delt_rand_1 = std(r_a_delt_p_delt_rand_all,[],1)./(sqrt(size(r_a_delt_p_delt_rand_all,1)));
...
patch([lags fliplr(lags)], [y_biceps_triceps_rand_1(:)-se_biceps_triceps_rand_1(:);  flipud(y_biceps_triceps_rand_1(:)+se_biceps_triceps_rand_1(:))],[0 0 0]/255, 'FaceAlpha',0.5, 'EdgeColor','none')
plot(lags,mean(r_biceps_triceps_rand_all),'LineWidth',2,'color',[0 0 0]/255)
set(gca,'TickDir','out')
set(gca,'box','off')
ylabel('Correlation')

r_biceps_triceps_all_fz = atanh(r_biceps_triceps_all);
r_biceps_trices_rand_all_fz = atanh(r_biceps_triceps_rand_all);
mean_r_biceps_triceps_rand_all_fz= mean(r_biceps_trices_rand_all_fz);
sd_r_biceps_triceps_rand_all_fz = std(r_biceps_trices_rand_all_fz,[],1);
r_biceps_triceps_all_z = (r_biceps_triceps_all_fz- mean_r_biceps_triceps_rand_all_fz)./sd_r_biceps_triceps_rand_all_fz;
for k = 1:size(r_biceps_triceps,2)
    [~,p(k)] = ttest2(r_biceps_triceps_all_fz(:,k),r_biceps_trices_rand_all_fz(:,k),'Vartype','unequal');
end
