...
for i = 1:length(index_js_reach) %nTrial
    ...
    if flag_noise(k) == 1 && path_length(j) < 3.5
        ...
        [r_bi_tri,lags] = xcorr(EMG_biceps-mean(EMG_biceps),EMG_tri-mean(EMG_tri),window_size*Fs_EMG/Fs_js,'coeff');
        ...
        r_bi_p_delt_all = [r_bi_p_delt_all r_p_delt_tri];
        ...
        [r_bi_js_pos,] = xcorr(EMG_bi_ds-mean(EMG_bi_ds),radial_pos-mean(radial_pos),window_size,'coeff');        
        ...
        [r_bi_js_vel,lags_js] = xcorr(EMG_a_delt_ds-mean(EMG_a_delt_ds),js_vel-mean(js_vel),window_size,'coeff');
        [r_tri_js_vel,~] = xcorr(EMG_triceps_ds-mean(EMG_triceps_ds),js_vel-mean(js_vel),window_size,'coeff');
        [r_a_delt_js_vel,] = xcorr(EMG_a_delt_ds-mean(EMG_a_delt_ds),radial_vel-mean(radial_vel),window_size,'coeff');        
        [r_p_delt_js_vel,] = xcorr(EMG_p_delt_ds-mean(EMG_p_delt_ds),radial_vel-mean(radial_vel),window_size,'coeff');        
        ...
        [r_bi_js_acc,~] = xcorr(EMG_a_delt_ds-mean(EMG_a_delt_ds),js_acc-mean(js_acc),window_size,'coeff');
        [r_tri_js_acc,~] = xcorr(EMG_triceps_ds-mean(EMG_triceps_ds),js_acc-mean(js_acc),window_size,'coeff');
        [r_a_delt_js_acc,] = xcorr(EMG_a_delt_ds-mean(EMG_a_delt_ds),radial_acc-mean(radial_acc),window_size,'coeff');        
        [r_p_delt_js_acc,] = xcorr(EMG_p_delt_ds-mean(EMG_p_delt_ds),radial_acc-mean(radial_acc),window_size,'coeff');        
        ...
        set(gca,'box','off')
        ax4 = subplot(5,1,4);
        plot4 = plot(time_EMG,EMG_a_delt,'LineWidth',1,'color','m');
        plot4.Color(4) = 0.5;
        hold on
        set(gca,'TickDir','out')
        set(gca,'box','off')
        ax5 = subplot(5,1,5);
        plot4 = plot(time_EMG,EMG_p_delt,'LineWidth',1,'color','g');
        plot4.Color(4) = 0.5;
        hold on
        set(gca,'TickDir','out')
        set(gca,'box','off')
