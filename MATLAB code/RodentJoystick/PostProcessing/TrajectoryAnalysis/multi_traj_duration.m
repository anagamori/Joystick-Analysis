function [console_output] = multi_traj_duration(dirlist,varargin)
...
for i= 1:length(statslist)
    ...
    [~,duration{i}] = traj_duration(stats,0,rw_only,interv,ax,plotflag,colors(i));    
    console_output{i+1} = sprintf(strcat(dates{i},' Med: %d'),median([duration{i}]));
end

console_output{1} = 'Duration';
if (lasercompareflag-1)
