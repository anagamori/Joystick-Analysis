function [console_output] = multi_seg_peakvel(dirlist,varargin)
...
if (lasercompareflag-1)
    ...
    [statslist, dates] = load_stats(dirlist, combineflag, to_stop, hr_in, {'traj_struct'});
    ...
    [statslist, dates] = load_stats(dirlist, combineflag, 0, to_stop,{'traj_struct'});
end

for i= 1:length(statslist)
    stats = get_stats_with_trajid(statslist(i),trajid);
    [~,peakvel{i}] = seg_peakvel(stats,0,rw_only,interv,ax,plotflag,colors(i));    
    console_output{i+1} = sprintf(strcat(dates{i},' Med: %d'),median([peakvel{i}]));
end

console_output{1} = 'Seg Peak Vel';
if (lasercompareflag-1)
    [h,p] = kstest2([peakvel{1}],[peakvel{2}]);
