function [seg_out] = get_seg_start_at(stats,rad_in,rad_out,seg_order,angle_in,angle_out,ccw)
...
%         
% try
%     loc_ind= find(stats.traj_struct(i).magtraj(locs_valley)>0&(locs_valley>1));
...
for i=1:num_traj
  if numel(stats.traj_struct(i).seginfo)
    ...
    for j = 1:numel(seg_index)    
        element = stats.traj_struct(i).seginfo(seg_index(j));
        ...
        if ccw
            if element_angle>angle_in && element_angle<angle_out
                seg_out = [seg_out stats.traj_struct(i).seginfo(seg_index(j))];
            end        
        else
            if element_angle<angle_in || element_angle>angle_out
                seg_out = [seg_out stats.traj_struct(i).seginfo(seg_index(j))];
